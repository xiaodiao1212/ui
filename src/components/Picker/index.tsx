...
const useStyles = createUseStyles<
  RuleNames,
  Pick<PickerProps, "cssOptions"> & { translateYlength: number[] },
  Theme
>((theme) => ({
...
const Picker = ({
  ...
  const [translateYlength, setTranslateYlength] = useState<number[]>(
    data.map((v) => 4)
  );
  ...
  const handleTouchMove = (
    ...
    const offsetY = parseFloat(
      ((e.touches[0].pageY - startPageY[index]) / 16).toFixed(1)
    );
    setOffsetY((v) =>
      v.map((v, i) => {
        if (i == index) {
  ...
  const handleTouchEnd = (index: number) => {
    ...
    let remainderPart = (os % 2).toFixed(1);

    let integerPart = Math.trunc(os);
    let decimalPart = os - integerPart;
